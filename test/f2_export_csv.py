...
for ds in dsName:
    ...
    path_lowpass = './filters/order1_{}_phi.pt'.format(ds)
    path_real = './filters/order1_{}_psi_real.pt'.format(ds)
    path_imagine = './filters/order1_{}_psi_imag.pt'.format(ds)
    ...
    save_filters(lowpass_rounded, 'csv/filter_{}_phi.csv'.format(ds))
    
    # export real and imaginary part to CSV file
    for k in range(8):
        save_filters(real_rounded[k,0], 'csv/filter_{}_psi_real_{}.csv'.format(ds, k))
        save_filters(imagine_rounded[k,0], 'csv/filter_{}_psi_imag_{}.csv'.format(ds, k))